...
class SelectorGroupView: UITableView, UITableViewDataSource, UITableViewDelegate {
    
    let model: BehaviorRelay<SelectorGroupModel>
    let tableHeight = BehaviorRelay<CGFloat>(value: 0)
    let answerRequestSubject = PublishSubject<AnswerRequest>()
    
    let disposeBag = DisposeBag()
    
    init(model: SelectorGroupModel) {
        self.model = BehaviorRelay<SelectorGroupModel>(value: model)
    ...
    private func setupUI() {
        register(SelectorCell.self, forCellReuseIdentifier: SelectorCell.reuseIdentifier)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: SelectorCell.reuseIdentifier, for: indexPath) as! SelectorCell
